...
outfile=$name"_"$size"_"$east"_"$west"_"$south"_"$north".nc"
...
mbgrid -I"file_list" -F2 -Ofoo -S0 -C2 -R$east/$west/$south/$north -N -E$size/0.0/m! -G3
...
mv foo.grd "$grd_dir/grd/$outfile"
