...
def mavs_cut_times(mavsid=1):
    """Based on visual inspection of time series, these are the start and end
    times for the MAVS moorings at depth

    Parameters
    ----------
    mavsid : int
        MAVS mooring id 1 through 4.

    Returns
    -------
    cut_beg : np.datetime64
        Start time
    cut_end : np.datetime64
        End time
    """
    if mavsid == 1:
        ...
        cut_beg = np.datetime64("2021-10-22 20:30:00")
        cut_end = np.datetime64("2022-08-04 13:45:00")
    elif mavsid == 4:
...
def rbr_blt1_load_mooring_sensor_info(thermistor_info_path):
    return blt1_load_mooring_sensor_info(thermistor_info_path, thermistor_type="rbr")


def rbr_blt2_load_mooring_sensor_info(thermistor_info_path):
    return blt2_load_mooring_sensor_info(thermistor_info_path, thermistor_type="rbr")
...
def blt2_rbr_cut_and_cal(sn, l0dir, l1dir, thermistor_info_path, ctdcal, end_manually, no_mavs_rbr=False):
    mavs3_rbr, mavs4_rbr = rbr_blt2_load_mooring_sensor_info(thermistor_info_path)
    if no_mavs_rbr:
        mavs3_rbr = mavs3_rbr.where(mavs3_rbr.Type != "MAVS Solo").dropna()
        mavs4_rbr = mavs4_rbr.where(mavs4_rbr.Type != "MAVS Solo").dropna()
    # get deployment times
    if sn in mavs3_rbr.index:
        cut_beg, cut_end = mavs_cut_times(mavsid=3)
    elif sn in mavs4_rbr.index:
        cut_beg, cut_end = mavs_cut_times(mavsid=4)
    else:
        print(f"cannot find SN{sn} in mooring info structure")
    print(f"loading SN{sn}")
    tmp = rbr_load_proc_level0(sn, l0dir)
...
def blt1_load_mooring_sensor_info(thermistor_info_path, thermistor_type="rbr"):
    ...
    if thermistor_type == "rbr":
        mavs1 = mavs1_info.query('Type=="Solo" | Type=="Solo Ti" | Type=="MAVS Solo"')
        mavs2 = mavs2_info.query('Type=="Solo" | Type=="Solo Ti" | Type=="MAVS Solo"')
...
def blt2_load_mooring_sensor_info(thermistor_info_path, thermistor_type="rbr"):
    mavs3_info_csv = thermistor_info_path.joinpath("blt_mavs3_thermistors_only.csv")
    mavs4_info_csv = thermistor_info_path.joinpath("blt_mavs4_thermistors_only.csv")

    mavs3_info = pd.read_csv(mavs3_info_csv, sep=",", header=0, index_col="SN")
    mavs4_info = pd.read_csv(mavs4_info_csv, sep=",", header=0, index_col="SN")

    if thermistor_type == "rbr":
        mavs3 = mavs3_info.query('Type=="Solo" | Type=="Solo Ti" | Type=="MAVS Solo"')
        mavs4 = mavs4_info.query('Type=="Solo" | Type=="Solo Ti" | Type=="MAVS Solo"')
    elif thermistor_type == "sbe":
        mavs3 = mavs3_info.query('Type=="SBE56"')
        mavs4 = mavs4_info.query('Type=="SBE56"')
...
class blt1_sbe56_proc:
    ...
    def load_paths(self):
        self.mooring_dir = Path("/Users/gunnar/Projects/blt/data/BLT/Moorings/BLT1/")
    ...
    def locate_gaps(self):
        ...
        # Find long gaps.
        long_gaps_all = {sn: find_first_long_gap(tdi) for sn, tdi in gaps.items()}
        long_gaps = {}
        ...
        _ = [print(sn, ":", ni) if ni > 0 else 0 for sn, ni in n_short_gaps.items()]
    ...
    def comment_no_data(self):
        sn_no_data = self.proc_info[~self.proc_info.raw_data_exists].index.to_numpy()
    ...
    def find_time_series_terminating_early(self):
        last_times = np.array([thi.time.isel(time=-1).data for thi in self.allnc])
    ...
    def load_level1(self, sn):
        return xr.open_dataarray(self.level1_dir.joinpath(f"blt1_sbe56_{sn:04}.nc"))
    ...
    def plot_all_level1(self):
        ...
        for sni in tqdm(allsn):
            ...
            sbe.sbe56.plot(th)
            plotname = self.figure_out_level1.joinpath(f"blt1_sbe56_{sni:04}_level1")
            gv.plot.png(plotname, verbose=False)
            plt.close()
            th.close()


class blt2_sbe56_proc:
    """Docstring for blt2_sbe56_proc."""

    def __init__(self):
        """Process BLT2 SBE56 data."""
        self.proc_info = self.generate_proc_info()
        self.load_paths()
        self.create_directories()
    ...
    def load_paths(self):
        self.mooring_dir = Path("/Users/gunnar/Projects/blt/data/BLT/Moorings/BLT1/")
        self.doc_dir = Path("/Users/gunnar/Projects/blt/proc/doc/")
        self.sbe_dir = self.mooring_dir.joinpath("MAVS/SBE56_2")
        self.data_raw = self.sbe_dir.joinpath("raw")
        self.data_out = self.sbe_dir.joinpath("proc")
        self.level1_dir = self.sbe_dir.joinpath("proc_L1")
        self.figure_out = self.sbe_dir.joinpath("fig")
    ...
    def generate_proc_info(self):
        cal_file = "blt2_sbe56_cal_info.csv"
        proc_info = pd.read_csv(
            cal_file,
            engine="python",
            header=0,
        ...
        proc_info.cal1 = pd.to_datetime(proc_info.cal1, errors="coerce", utc=True)
        proc_info.cal2 = pd.to_datetime(proc_info.cal2, errors="coerce", utc=True)
        proc_info.ctd_cal = pd.to_datetime(proc_info.ctd_cal, errors="coerce", utc=True)
    ...
    def get_file_name(self, sn, dir="raw", extension="csv"):
        if dir == "raw":
            dir = self.data_raw
        files = list(dir.glob(f"SBE056{sn:05g}*.{extension}"))
        if len(files) == 1:
            return files[0]
        elif len(files) == 0:
            return None
    ...
    def find_time_series_terminating_early(self):
        last_times = np.array([thi.time.isel(time=-1).data for thi in self.allnc])
        ind_bool = last_times < np.datetime64("2022-08-06 00:00:00")
        # Extract short time series
        short = [d for d, s in zip(self.allnc, ind_bool) if s]
        sn = [ti.attrs["SN"] for ti in short]
        time = [ti.time[-1].data for ti in short]
    ...
    def comments_to_latex_table(self):
        self.proc_info.to_latex(
            self.doc_dir.joinpath("blt2_sbe56_table.tex"),
            columns=[
                "Notes",
                "comment",
            ],
    ...
    def load_mooring_sensor_info(self):
        self.mavs3, self.mavs4 = blt2_load_mooring_sensor_info(
            self.thermistor_info_path, thermistor_type="sbe"
        )
    ...
    def cut_and_cal(self, sn):
        # Load thermistor data and apply CTD calibration offset
        th = self.calibrate(sn)
        if sn in self.mavs3.index:
            cut_beg, cut_end = mavs_cut_times(mavsid=3)
        elif sn in self.mavs4.index:
            cut_beg, cut_end = mavs_cut_times(mavsid=4)
        # See if we need to cut short because of a gap > 1h.
        if sn in self.first_long_gap:
            cut_end = self.first_long_gap[sn].time.data
    ...
    def generate_level1_file(self, sn):
        th = self.cut_and_cal(sn)
        savename = self.level1_dir.joinpath(f"blt2_sbe56_{sn:04}.nc")
        th.to_netcdf(savename)
    ...
    def load_level1(self, sn):
        return xr.open_dataarray(self.level1_dir.joinpath(f"blt2_sbe56_{sn:04}.nc"))
    ...
    def plot_all_level1(self):
        ...
        for sni in tqdm(allsn):
            th = self.load_level1(sni)
            sbe.sbe56.plot(th)
            plotname = self.figure_out_level1.joinpath(f"blt2_sbe56_{sni:04}_level1")
            gv.plot.png(plotname, verbose=False)
            plt.close()
            th.close()
